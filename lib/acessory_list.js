var SelectAcessories = React.createClass({
  ...
  render: function(){
    return (
      <button id = 'select-acesssories-btn' className="btn btn-success sucess" type="submit" onClick={this.selectAcessories}>Salvar Desenho<span className="glyphicon glyphicon-ok-sign" aria-hidden="true"></span></button>
    );
  }
});
 ...
 render: function(){
    return (
      <div className="acessory-item">
      	<div value={this.props.name} className= 'col-lg-6 acessory-name'>{ACESSORIES_DISPLAY_NAMES[this.props.name]}</div>
  ...
  componentDidMount: function(){
	...
	render: function() {
	    ...
	    return (
          ...
          <div className = 'col-lg-4 qtd-title'>
            Qtd
          </div>
          <div className = 'col-lg-2'>
            Remover
