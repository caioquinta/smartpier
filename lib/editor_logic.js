...
checkConnectability = function() {
  ...
  if (connectors.length > 0 ) {
    for(var i=0; i < connectors.length; i++) {
      ...
      //console.log('ponto: '+ connectors[i].item.x +','+ connectors[i].item.y +'is_connected '+ connectors[i].item.is_connected );
    }
  }
  return connectors_qtd;
}
...
drawConnectors = function(container, type) {
  ...
  switch(type) {
    case 'pier_gd':
      drawHorizontalConnectors(piece.x + METER_SCALE/3, piece.x + piece.getBounds().width, piece.y); //aresta superior
      //drawHorizontalConnectors(container, piece.y + METER_SCALE/3, piece.y + piece.getBounds().height, false, piece.x -1);
      drawHorizontalConnectors(piece.x + METER_SCALE/3, piece.x + piece.getBounds().width, piece.getBounds().height + piece.y); //aresta inferior
      break;
    case 'pier_pq':
      console.log('pier_pq');
      break;
...
drawHorizontalConnectors = function(initial_pos, end_pos, fixed_pos) {
  for(var i = initial_pos; i < end_pos; i= i + METER_SCALE/2 ){
    ...
    rect.y = fixed_pos;
    stage.addChild(rect);
    connector = {'item': rect, 'is_connected': false};
