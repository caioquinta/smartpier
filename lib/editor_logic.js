...
drawPiece = function (_, pier_name) {
  var container = new createjs.Container();
  var bmp = new createjs.Bitmap(queue.getResult(pier_name + '_piece'));
  container.addChild(bmp);
  bmp.x = 100;
  bmp.y = 100;
  container.on("dblclick", function() { rotate(bmp); });
...
drawMenu = function () {
  ...
  addButton('drawPiece', null, 'pier_gd', 5, container);
  addButton('drawPiece', null, 'pier_pq', BUTTON_HEIGHT + 10, container);
  addButton('drawPiece', null, 'apoio', 2*BUTTON_HEIGHT + 15, container);
  addButton('drawPiece', null, 'suporte_estaca', 3*BUTTON_HEIGHT + 20, container);  
  addButton('toogleDelete', state, 'trash_can', 6*(BUTTON_HEIGHT) + 35, container);
  stage.addChild(container);
  stage.update();
}

addButton = function  (action_name, args, pier_name, posy, container) {
  ...
  bmp.x = 505;
  bmp.y = posy;
  bmp.on("click", function() { action(args, pier_name);});
