...
drawPierDefaultConnectors = function(piece) {
  if (Math.round(Math.sin(toRadians(piece.rotation)))== 0 && Math.round(Math.cos(toRadians(piece.rotation)))==1){
  ...
  else if (Math.sin(piece.rotation)== -1 && Math.cos(piece.rotation)==0) {
  }
    ...
    drawPieceConnectors(piece.y + METER_SCALE/3, piece.y + piece.getBounds().width,  piece.getBounds().height + piece.x, false); //aresta inferior
  }
}
