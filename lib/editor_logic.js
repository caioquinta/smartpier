...
toogleDelete = function () {
  ...
  if (state) {
    ...
    $.each(buttons, function( key, value ) {
      value.bmp.removeAllEventListeners('click');
      //value.bmp.off("click", buttons[key].btn_click);
      value.bmp.alpha = 0.5;
    });
    ...
    $.each(buttons, function( key, value ){
      value.bmp.on("click", buttons[key].btn_click);
      value.bmp.alpha = 1;
    });
  }
  stage.update();
...
addButton = function  (action_name, is_continuous, pier_name, posy, container) {
  ...
  btn_event = bmp.on("click", function() { btnClick(action, is_continuous, pier_name, bmp); });
  btn_container.addChild(bmp);
  btn_container.addChild(rect);
  container.addChild(btn_container);
  if (!is_continuous) buttons.push({'bmp': bmp, 'btn_click': btn_event });
}
