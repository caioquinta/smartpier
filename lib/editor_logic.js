  ...
  if (connectors.length > 0 ) {
    for(var i=0; i < connectors.length; i++) {
      for(var j=i+1; j < connectors.length; j++){
        ...
        if (is_connected && (connectors[j].name == 'apoio' ||  connectors[i].name == 'apoio'  ) ) mother_connectors_qtd++;
      }
    }
  }
  return {'connectors_qtd': connectors_qtd, 'mother_connectors_qtd': mother_connectors_qtd };
}
...
drawConnectors = function(container, type) {
  ...
  switch(type) {
    ...
    case 'pier_pq':
      ...
      break;
    case 'apoio':
      drawPierTriangleConnectors(piece, type);
      break;    
    case 'suporte_estaca':
...
drawPierDefaultConnectors = function(piece) {
  if (Math.round(Math.sin(toRadians(piece.rotation)))== 0 && Math.round(Math.cos(toRadians(piece.rotation)))==1){
    ...
    drawPieceConnectors(piece.y - piece.getBounds().width + METER_SCALE/3, piece.y,  piece.x, false); //aresta superior
    drawPieceConnectors(piece.x+ METER_SCALE/3, piece.x + piece.getBounds().height , piece.y, true); //lateral esquerda
    drawPieceConnectors(piece.y - piece.getBounds().width + METER_SCALE/3, piece.y,  piece.x + piece.getBounds().height, false); //aresta inferior
    drawPieceConnectors(piece.x+ METER_SCALE/3, piece.x + piece.getBounds().height , piece.y - piece.getBounds().width, true); //lateral direita
  }
  else if (Math.round(Math.sin(toRadians(piece.rotation)))== 1  && Math.round(Math.cos(toRadians(piece.rotation)))== 0) {
...
drawPierTriangleConnectors = function(piece, type) {
  if (Math.round(Math.sin(toRadians(piece.rotation)))== 0 && Math.round(Math.cos(toRadians(piece.rotation)))==1){
    drawPieceConnectors(piece.x + METER_SCALE/3, piece.x + piece.getBounds().width,  piece.y, true, type); //aresta superior
    drawPieceConnectors(piece.y + METER_SCALE/3, piece.y + piece.getBounds().height, piece.x, false, type); //lateral esquerda
  }  
  else if (Math.round(Math.sin(toRadians(piece.rotation)))== -1 && Math.round(Math.cos(toRadians(piece.rotation)))== 0){
    drawPieceConnectors(piece.y - piece.getBounds().width + METER_SCALE/3, piece.y,  piece.x, false, type); //aresta superior
    drawPieceConnectors(piece.x+ METER_SCALE/3, piece.x + piece.getBounds().height , piece.y, true, type); //lateral esquerda
  }
  else if (Math.round(Math.sin(toRadians(piece.rotation)))== 0 && Math.round(Math.cos(toRadians(piece.rotation)))== -1){
    drawPieceConnectors(piece.x - piece.getBounds().width + METER_SCALE/3, piece.x,  piece.y, true, type); //aresta superior
    drawPieceConnectors(piece.y - piece.getBounds().height + METER_SCALE/3, piece.y, piece.x, false, type); //lateral esquerda
  }
  else if (Math.round(Math.sin(toRadians(piece.rotation)))== 1  && Math.round(Math.cos(toRadians(piece.rotation)))== 0) {
    drawPieceConnectors(piece.x - piece.getBounds().height + METER_SCALE/3, piece.x, piece.y, true, type); //lateral esquerda
    drawPieceConnectors(piece.y + METER_SCALE/3, piece.y + piece.getBounds().width,  piece.x, false, type); //aresta superior
  }
}
...
drawPieceConnectors = function(initial_pos, end_pos, fixed_pos, is_horizontal, type) {
