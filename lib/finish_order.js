...
var OrderForm = React.createClass({
  ...
  handleSubmit: function(e) {
    ...
    $.map(itensByOcurrency(piers), function( value, key ) { piers_selected.push({ name: PIERS_DISPLAY_NAMES[key], qtd: value })});
    this.state['piers_selected'] = piers_selected;
    
    $('.acessory-item .acessory-name').map(function(){ itens_selected.push({name: $(this).text(), qtd: $('#'+ $(this).attr('value') + '-item .qtd').text()})}).get();
    this.state['itens'] = itens_selected;
    ...
    this.state['pier_image'] =  canvas.toDataURL();
    ...
    <div>
      <div className = 'order-list col-lg-6' >
        <div className = 'col-lg-12'>
          ...
          <h5>Conectores para os Piers </h5>
          <div className = 'order-itens panel-body'>
            { $.map(calculate_connector_parts(this.state.connectors.connectors_qtd, this.state.connectors.mother_connectors_qtd ), function(qtd, name){
              return ( 
                <div>
                  <div className = 'col-lg-10'>
                    {name}
